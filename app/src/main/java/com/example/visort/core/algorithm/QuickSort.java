...
public class QuickSort {
    ...
    private final Handler quickHandler;

    Runnable quickRunnable = new Runnable() {
        @Override
        public void run() {
            try {
                initializeThread();
                ...
                quickHandler.post(() -> Toast.makeText(mContext, e.getMessage(), Toast.LENGTH_SHORT).show());
            } finally {
                finalizeThread();
            }
        }
    ...
    public QuickSort(BarChart barChart, ArrayList<BarEntry> barEntryArrayList, ArrayList<String> label_names, ArrayList<BarchartModel> barChartModelArrayList, Slider slider, Context mContext) {
        ...
        quickHandler = new Handler();
        new Thread(quickRunnable).start();
    }
...
//        sliderSpeed.setVisibility(View.GONE);
    ...
    int quickPartition(ArrayList<Integer> data, int lowerIndex, int highIndex) throws InterruptedException {
        int pivot = data.get(highIndex);
        int i = lowerIndex - 1;
        for (int j = lowerIndex; j < highIndex; j++) {
            if (data.get(j) < pivot) {
                i++;
                int temp = data.get(i);
                data.set(i, data.get(j));
                data.set(j, temp);
                quickHandler.post(() -> converterToModel(data));
                sleep(ThreadState.delayTime);
            }
        }
        int temp = data.get(i + 1);
        data.set(i + 1, data.get(highIndex));
        data.set(highIndex, temp);
        quickHandler.post(() -> converterToModel(data));
        sleep(ThreadState.delayTime);
    ...
    void quickSort(ArrayList<Integer> data, int lowerIndex, int highIndex) throws InterruptedException {

        if (lowerIndex < highIndex) {
            int pi = quickPartition(data, lowerIndex, highIndex);
            quickSort(data, lowerIndex, pi - 1);
            quickHandler.post(() -> converterToModel(data));
            sleep(ThreadState.delayTime);
            quickSort(data, pi + 1, highIndex);
            quickHandler.post(() -> converterToModel(data));
            sleep(ThreadState.delayTime);
        }
    }
